...
def cost_function(X, y, W, bias, lam):
    ...
    return 0.5 * (1 / m) * np.sum((y - y_hat) ** 2) + lam / (2 * n) * np.sum(W ** 2)
...
if __name__ == '__main__':
    ...
    plt.plot(range(iterations), costs_train, label='train loss')
    plt.plot(range(iterations), costs_test, label='test loss')
