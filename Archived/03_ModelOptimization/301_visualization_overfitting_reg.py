...
def prediction(x_test, y_test, x_train, y_train):
    ...
    plt.plot(x_test, y_pre_1, c='brown', label='degree = 1, r2 = {}'.format(round(r2_1, 2)))
    plt.plot(x_test, y_pre_5, c='green', label='degree = 5, r2 = {}'.format(round(r2_5, 2)))
    plt.plot(x_test, y_pre_10, c='orange', label='degree = 10, r2 = {}'.format(round(r2_10, 2)))
    plt.tight_layout()  # 调整子图间距
    plt.legend()
    plt.show()
...
def train(x_train, y_train):
    ...
    plt.scatter(x_train, y_train, c='red', label='training data')
    plt.plot(x_train, y_pre_1, c='brown', label='degree = 1, r2 = {}'.format(round(score_1, 2)))
    plt.plot(x_train, y_pre_5, c='green', label='degree = 5, r2 = {}'.format(round(score_5, 2)))
    plt.plot(x_train, y_pre_10, c='orange', label='degree = 10, r2 = {}'.format(round(score_10, 2)))
    plt.tight_layout()  # 调整子图间距
    plt.legend()
    plt.show()
