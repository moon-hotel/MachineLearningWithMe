...
def plot_countour():
    ...
    for i in range(28):  # 梯度反方向，最速下降曲线
        q = f_grad(p[0], p[1])
        print("P{}:{}".format(i, p))
        ax.arrow(p[0], p[1], q[0], q[1], head_width=0.1, head_length=0.1, fc='r', ec='r')
        p += q  # 上一次的位置加上本次的梯度
    plt.annotate("反梯度方向,最速下降曲线", xy=(-4.7, -0.5), fontsize=12, c='r')

    p = np.array([0.3, 0.01])  # 起始位置
    for i in range(20):  # 梯度方向，最速上升曲线
        q = f_grad(p[0], p[1], descent=False)
        ax.arrow(p[0], p[1], q[0], q[1], head_width=0.1, head_length=0.05, fc='b', ec='b')
        p += q  # 上一次的位置加上本次的梯度
    plt.annotate("梯度方向,最速上升曲线", xy=(0.5, -0.5), fontsize=12, c='b')
