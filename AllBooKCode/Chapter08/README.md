## Code Index

### [Understanding Decision Trees and Ensemble Models (in Chinese)](https://mp.weixin.qq.com/s/UZsKvsdbSD6OT3oMhWT1Dw)

- [Visualization of Binary-Classification Information Entropy](01_visualization.py)
- [Feature Importance of CART](02_decision_tree_gini.py)
- [Bagging Ensemble Learning for KNN](03_ensemble_bagging_knn.py)
- [AdaBoost with Decision Trees](04_AdaBoosting.py)
- [AdaBoost with GridSearchCV](05_AdaBoosting_gridsearch.py)
- [Stacking Ensemble Learning](06_ensemble_stacking.py)
- [Stacking with GridSearchCV](07_ensemble_stacking_gridsearch.py)
- [Random Forest](08_ensemble_random_forest.py)
- [Computing Feature Importance](09_ensemble_random_forest_features.py)
- [Feature Importance with Random Forest](10_feature_importance_comp.py)
- [Titanic Classification by Random Forest](11_titanic.py)
- [Visualization of Decision Trees](12_visiualization_dt.py)

### [Introduction of ID3 and C4.5 Implementation from Scratch (in Chinese)](https://mp.weixin.qq.com/s/ZV2rLuRaF-pFwmBWMfBsiw)

- [Implementation of ID3 and C4.5 from Scratch](C13_id3_categorical.py)
- [Implementation of ID3 and C4.5 from Scratch (Continuous Features)](C14_id3_continuous.py)
- [Implementation of CART from Scratch](C15_cart_imp.py)

### [Understanding AdaBoost Algorithm from Scratch (in Chinese)]()

- [Implementation of AdaBoost](C17_manual_adaboost.py)

### [Understanding AdaBoost with SAMME (in Chinese)]()

- [Model Error vs Model Weights in AdaBoost](C16_adaboost_plot.py)
- [Implementation of AdaBoost with SAMME](C18_adaboost_imp.py)

### [Understanding GradientBoost Algorithm from Scratch (in Chinese)]()

- [GradientBoostDecisionTree in Sklearn](./C20_gbdt_train.py)
- [Implementation of GradientBoost Regression](./C21_manaual_gradient_boost_reg.py)

### [Home](../../README.md)&nbsp;&nbsp;&nbsp;&nbsp;[Back to KnowledgeIndex](../Index.md)